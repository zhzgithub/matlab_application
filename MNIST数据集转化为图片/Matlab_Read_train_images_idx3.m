...
% 图片以TrainImage_00001.bmp～TrainImage_60000.bmp的格式保存在指定路径，60000个文件占用空间39M。。
% 整个程序运行过程需几分钟时间。
...
%从上面提到的网页可以理解这四句
%读取测试labels文件
[FileName1,PathName1] = uigetfile('*.*','选择测试图片数据文件train-label.idx1-ubyte');
...
if ((MagicNum1~=2049)||(ImageNum1~=60000))
...
for i=1:ImageNum
    ...
    switch b1
        case 0
            savepath = fullfile(savedirectory,'num0',['TrainImage_' num2str(i,'%d') '.bmp']);
            imwrite(e,savepath,'bmp'); %最后用imwrite写出图片
        case 1
            savepath = fullfile(savedirectory,'num1',['TrainImage_' num2str(i,'%d') '.bmp']);
            imwrite(e,savepath,'bmp'); %最后用imwrite写出图片
        case 2
            savepath = fullfile(savedirectory,'num2',['TrainImage_' num2str(i,'%d') '.bmp']);
            imwrite(e,savepath,'bmp'); %最后用imwrite写出图片
        case 3
            savepath = fullfile(savedirectory,'num3',['TrainImage_' num2str(i,'%d') '.bmp']);
            imwrite(e,savepath,'bmp'); %最后用imwrite写出图片
        case 4
            savepath = fullfile(savedirectory,'num4',['TrainImage_' num2str(i,'%d') '.bmp']);
            imwrite(e,savepath,'bmp'); %最后用imwrite写出图片
        case 5
            savepath = fullfile(savedirectory,'num5',['TrainImage_' num2str(i,'%d') '.bmp']);
            imwrite(e,savepath,'bmp'); %最后用imwrite写出图片
        case 6
            savepath = fullfile(savedirectory,'num6',['TrainImage_' num2str(i,'%d') '.bmp']);
            imwrite(e,savepath,'bmp'); %最后用imwrite写出图片
        case 7
            savepath = fullfile(savedirectory,'num7',['TrainImage_' num2str(i,'%d') '.bmp']);
            imwrite(e,savepath,'bmp'); %最后用imwrite写出图片
        case 8
            savepath = fullfile(savedirectory,'num8',['TrainImage_' num2str(i,'%d') '.bmp']);
            imwrite(e,savepath,'bmp'); %最后用imwrite写出图片
        case 9
            savepath = fullfile(savedirectory,'num9',['TrainImage_' num2str(i,'%d') '.bmp']);
            imwrite(e,savepath,'bmp'); %最后用imwrite写出图片
    end
    waitbar(i/ImageNum);
end
